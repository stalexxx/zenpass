...
|---|---|---|---|
| G-01 | **Delivered 2026-09-04:** `packages/crypto-fixtures/` is a dependency-free Bun harness (encodings, normalize/digest, manifest, loader, CLI) per `H01-CRYPTO-FIXTURES-VERIFIER-PLAN.md`. `bun run --filter crypto-fixtures test:structure` (6 fixtures, 29 adapter cases) and `test:unit` (28 tests) pass; `test` (full, adapter-backed) correctly fails closed with no adapter configured. It performs no cryptographic operation itself. The reviewer's independent adapter run remains G-05, separately. | AC-5 | A04 rework complete; H01 reviewer runs the independent adapter (G-05) |
| G-02 | **Delivered:** `fixtures/crypto/aead-xchacha20poly1305.json` carries an authoritative positive XChaCha20-Poly1305 vector (from `draft-irtf-cfrg-xchacha`) plus tamper-rejection negatives (ciphertext/tag/nonce/AAD byte flips). Note: its AAD is the source document's test AAD, not a `crypto-envelope/v1` canonical AAD — contract-context binding is exercised only by the still-blocked G-03 wrap vectors. Structurally validated by the crypto-fixtures harness (G-01); independent cryptographic re-derivation is G-05, separate human reviewer work. | AC-5, AC-2 | A04 rework complete for this fixture; G-05 independent verification remains |
| G-03 | No key-wrapping vectors (account/recovery/vault/item). **Explicitly blocked, not merely pending**: no authoritative external source defines this contract's wrapper AAD/kinds (they are application-specific to `crypto-envelope/v1`); a human H01 reviewer must approve a fixture-generation procedure and an independently implemented validator adapter before any wrap vector may be added (full reasoning recorded in `docs/security/G03-WRAP-VECTORS-BLOCKER.json`; kept out of `fixtures/crypto/` since the crypto-fixtures harness (G-01) validates that directory strictly against a closed manifest and this is a blocker record, not a vector). | AC-5 | Human H01 reviewer decision required before A04 rework can proceed |
| G-04 | **Delivered:** `fixtures/crypto/opaque-3dh-ristretto255.json` carries an OPAQUE-3DH Ristretto255 registration/login transcript vector (from RFC 9807) plus tamper/mismatch negatives. Structurally validated by the crypto-fixtures harness (G-01); independent verification against the chosen RFC 9807 validator is G-05. | AC-5 | A04 rework complete for this fixture; G-05 independent verification remains |
...
| G-09 | **Delivered 2026-09-04:** `recovery-semantics-01`'s placeholder ciphertext is resized from 56 B to the contract-correct 48 B (32 B wrapped AccountKey + 16 B tag), and now explicitly marked `ciphertextIsPlaceholder`/`placeholderNote` so it cannot be mistaken for a positive wrap vector. Still not a real AEAD/wrap vector (that's G-03, blocked) — only the placeholder's length and labeling are corrected. | AC-4, AC-5 | A04 rework complete |
...
| G-11 | **Fixture defect corrected 2026-09-03:** the positive `canonicalCborHex` now encodes outputLength 32 as `0x18 0x20`; the historical `0x20` bytes are retained as a semantic `InvalidKdfParameters` regression because they canonically encode −1. A separate true non-minimal-integer case expects `NonCanonicalCbor`. Independent adapter-backed verification remains required under G-05. | AC-2, AC-5 | A04 fixture rework complete for the bytes; H01 reviewer verifies via independent adapter |
