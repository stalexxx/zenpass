...
| B05 | READY | sync agent | B02,B04,A02 | Integrator judgment call: promoted ahead of B04's full MERGED status because the only B04 surface B05 needs — working bearer sessions via `/auth/opaque/login` + `requireSession` — is implemented and tested; B04's remaining gaps (WebAuthn, recovery-reset) touch no sync endpoint. `docs/tasks/B05.md` still names `apps/api/src/sync/`, a stale path predating the B02 `apps/api/` → `apps/backend/` correction (`infra/B02-INTEGRATION-ESCALATION.md`); the sync agent should use `apps/backend/src/sync/` and `apps/backend/test/sync/` instead. |
